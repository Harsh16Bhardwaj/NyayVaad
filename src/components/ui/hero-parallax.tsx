...
const offerings = [
  {
    title: "Instant Case Consultation",
    desc: "Upload or describe your issue. Our AI delivers a clear, structured case summary in minutes.",
    icon: <MessageSquare className="w-8 h-8 text-teal-400" />,
    color: "bg-teal-800/70",
  },
  {
    title: "Enriched Legal Intelligence",
    desc: "Combines your profile, AI analysis, and verified legal cases for a complete view.",
    icon: <Brain className="w-8 h-8 text-amber-400" />,
    color: "bg-amber-900/70",
  },
  {
    title: "No Jargon, Just Clarity",
    desc: "Laws and judgments explained in plain English, so you understand the why.",
    icon: <BookOpen className="w-8 h-8 text-purple-400" />,
    color: "bg-purple-900/70",
  },
  {
    title: "Dynamic Action Plan",
    desc: "Get tailored recommendations and to-do tasks to move your case forward.",
    icon: <List className="w-8 h-8 text-blue-400" />,
    color: "bg-blue-900/70",
  },
  {
    title: "Confidential. Always.",
    desc: "Encrypted analysis, no storage without consent, ensuring your privacy.",
    icon: <Lock className="w-8 h-8 text-green-400" />,
    color: "bg-green-900/70",
  },
];

// Repeat offerings to fill 3 rows of 4 cards each (12 cards total)
const parallaxCards = [
  ...offerings.slice(0, 4), // First row: Instant, Enriched, No Jargon, Dynamic
  ...offerings.slice(4).concat(offerings.slice(0, 3)), // Second row: Confidential, Instant, Enriched, No Jargon
  ...offerings.slice(3).concat(offerings.slice(0, 2)), // Third row: Dynamic, Confidential, Instant, Enriched
];

const onboardingSteps = [
  {
    title: "Sign Up & Verify",
    desc: "In under 30 seconds.",
    icon: <Lock className="w-8 h-8 text-teal-400" />,
    img: "/placeholder-step1.png",
  },
  {
    title: "Explore AI Tools",
    desc: "Unleash legal superpowers.",
    icon: <Brain className="w-8 h-8 text-teal-400" />,
    img: "/placeholder-step2.png",
  },
  {
    title: "Analyze Cases",
    desc: "Dive into precedents.",
    icon: <BookOpen className="w-8 h-8 text-teal-400" />,
    img: "/placeholder-step3.png",
  },
  {
    title: "Track Progress",
    desc: "Stay ahead of the game.",
    icon: <List className="w-8 h-8 text-teal-400" />,
    img: "/placeholder-step4.png",
  },
  {
    title: "Download Reports",
    desc: "Win with structured insights.",
...
const howWeWorkPoints = [
  {
    text: "Summarize cases into one-liners with Langflow API.",
    icon: <Workflow className="w-8 h-8" />,
  },
  {
    text: "Retrieve precedents from Indian Kanoon’s database.",
    icon: <Database className="w-8 h-8" />,
  },
  {
    text: "Refine data with ChatGPT for deeper insights.",
    icon: <MessageSquare className="w-8 h-8" />,
  },
  {
    text: "Analyze with Gemini’s 32k token model.",
    icon: <Brain className="w-8 h-8" />,
  },
  {
    text: "Secure data with Supabase and Clerk auth.",
    icon: <Lock className="w-8 h-8" />,
  },
  {
    text: "Deliver structured outputs via HTML parsing.",
...
const HeroParallax = ({ products }: { products: typeof offerings }) => {
...
const Header = () => {
  return (
    <div className="max-w-7xl relative mx-auto py-20 md:py-40 px-4 w-full left-0 top-0">
      <h1 className="text-5xl md:text-6xl font-bold text-[#1CEFFF] font-inter">
        What We Offer
      </h1>
      <p className="max-w-3xl text-lg md:text-xl mt-8 text-gray-300">
        NyayVaad delivers AI-powered legal solutions that are clear, actionable, and secure, empowering you to tackle any legal challenge with confidence.
      </p>
    </div>
  );
};

// ProductCard Component (now rendering cards instead of images)
const ProductCard = ({
  product,
  translate,
}: {
  product: typeof offerings[0];
  translate: any;
}) => {
  return (
    <motion.div
      ...
      role="article"
      aria-labelledby={`card-${product.title}`}
    >
      <div className={`absolute inset-0 h-full w-full ${product.color} p-6 flex flex-col justify-start items-start`}>
        <div className="mb-4">{product.icon}</div>
        <h2 id={`card-${product.title}`} className="text-xl font-semibold text-white mb-2">
          {product.title}
        </h2>
        <p className="text-sm text-gray-200 max-w-xs">{product.desc}</p>
      </div>
      <div className="absolute inset-0 h-full w-full opacity-0 group-hover/product:opacity-30 bg-black pointer-events-none transition-opacity duration-300"></div>
    </motion.div>
  );
};
  ...
  return (
    <div
      className={`min-h-screen ${
        darkMode ? "bg-[#1F1F1F]" : "bg-[#F8F9FA]"
      } font-inter transition-colors duration-300 overflow-x-hidden`}
      style={
        {
          "--gradient-start": "#3B0764",
          "--gradient-end": "#1CEFFF",
          "--coral-600": "#FF6B6B",
          "--coral-400": "#FF8787",
        } as any
    ...
    >
      <main>
        ...
        <motion.section
          ...
          style={{
            background: "linear-gradient(45deg, #360033,#cc5333,#0b8793)",
        ...
        >
          ...
          <div className="max-w-6xl mx-auto px-4 text-center">
            <motion.h1
              id="hero-heading"
              className="text-6xl md:text-6xl font-bold text-white font-inter mb-6 leading-tight"
              initial={{ opacity: 0, y: 50 }}
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8, staggerChildren: 0.3 }}
            >
              {["Facing legal trouble,", "Not sure where to start?"].map(
                (line, index) => (
            ...
            <motion.p
              ...
              } max-w-2xl mx-auto font-semibold mb-8 `}
              initial={{ opacity: 0, y: 50 }}
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8, delay: 0.9 }}
            >
              Legal confusion ends here.
              <span className="block font-normal text-gray-300/90">
                Get clear, AI-powered insights in minutes — not weeks. No
                appointments, no paperwork, just clarity.
              </span>
            </motion.p>
            <motion.div
              initial={{ opacity: 0, y: 50 }}
            ...
            >
              <Link href="/dashboard">
                <button
                  className="bg-[#FF6B6B] cursor-pointer hover:scale-103 duration-150 ease-in-out hover:duration-150 hover:ease-in-out mt-5 text-white px-12 py-5 rounded-full text-xl font-semibold hover:bg-[#ff7979]  transition-all [filter:drop-shadow(0_0_10px_rgba(255,107,107,0.6))]"
            ...
            >
              <button
                ...
                className="text-[#1CEFFF] hover:text-teal-300 transition-colors"
        ...
        {/* What We Offer Section (Replaced with HeroParallax) */}
        <motion.section
          ref={(el) => (sectionRefs.current[1] = el)}
          className={`min-h-screen ${darkMode ? "bg-[#1F1F1F]" : "bg-[#F8F9FA]"} relative`}
          initial="hidden"
          whileInView="visible"
          viewport={{ once: false, amount: 0.3 }}
          variants={fadeIn}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                ...
                },
                {
                  ...
                  text: "Secure data with Supabase.",
                },
                {
                  logo: "/kanoon-logo.png",
              ...
              {onboardingSteps.map((step, index) => (
                <motion.div
                  ...
                  className={`flex flex-col items-center bg-${
                    darkMode ? "[#1a2632]/80" : "[#FFFFFF]"
                  } p-6 rounded-xl border ${
                    darkMode ? "border-teal-800/30" : "border-gray-200"
                  } w-full md:w-1/5`}
        ...
        >
          <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row gap-8">
            ...
            >
              <img
                src="/flowchart.png"
                alt="NyayVaad Legal workflow diagram showing the process from user inputs to final AI case analysis using Langflow API, Indian Kanoon, ChatGPT, Gemini, and vector databases."
                className="w-full rounded-lg"
              />
            </motion.div>
          </div>
        ...
        >
          ...
          <div className="max-w-6xl mx-auto px-4 text-center relative z-10">
            ...
            >
              <Link href="/dashboard">
                <button
                  className="bg-[#FF6B6B] text-white px-12 py-5 rounded-full text-xl font-semibold hover:bg-[#FF8787] transition-all [filter:drop-shadow(0_0_10px_rgba(255,107,107,0.6))]"
                  aria-label="Start your free analysis"
                >
                  Start Free Analysis
                </button>
              ...
              <Link href="/pricing">
                <button
                  className="border border-[#1CEFFF] text-[#1CEFFF] px-12 py-5 rounded-full text-xl font-semibold hover:bg-[#1CEFFF]/10 transition-all"
        ...
        <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row justify-between items-center">
          ...
          <div className="flex gap-6">
            <Link
              ...
              } hover:text-[#1CEFFF] transition-colors`}
